...
class SubmitNickname extends React.Component<{}, SubmitNicknameState> {
  ...
  textAreaChange(event) {
    ...
    this.setState({
      ...
      submitActive: this.state.charactersRemaining < 0 ? false : true
    });


  }

  speciesComplete(event) {

  }

  render() {
    let matchToTerm = (species, value) => {
      return species.toLowerCase().indexOf(value.toLowerCase()) !== -1;
    }
    return (
      <div className='submit-nickname-wrapper'>
        ...
        <form action='' method='post' enctype='multipart/form-data' className='submit-nickname-form'>
          ...
          <input disabled={!this.state.submitActive ? true : false} className='submit-button' type='submit' value='submit' />
        </form>
      </div>
    )
  }
